...
EnemyTypes getEnemyType(string url) {
    if (url == Maps::filePrefix + "enemyTank-hard.png") {
        return EnemyTypes::Hard;
    }
    if (url == Maps::filePrefix + "enemyTank-stationary.png") {
        return EnemyTypes::Stationary;
    }
    if (url == Maps::filePrefix + "enemyTank-boss.png") {
...
void EnemyTank::update(sf::RenderWindow* window, sf::Event& event, vector<shared_ptr<Sprite>>& allSprites, sf::Clock& clock) {
    switch (getEnemyType(getUrl()))
    {
        ...
        {
            basicUpdate(window, event, allSprites, clock);
            break;
        }
        case EnemyTypes::Hard:
        {
            hardUpdate(window, event, allSprites, clock);
            break;
        }
        case EnemyTypes::Stationary:
        {
            stationaryUpdate(window, event, allSprites, clock);
            break;
        }
        case EnemyTypes::Boss:
        {
            bossUpdate(window, event, allSprites, clock);
            break;
        }
    }
}
